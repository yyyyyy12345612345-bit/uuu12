...
const TRAINING_CORPUS: MLDocument[] = [
  ...
  },
  {
    ...
    getReply: (userName) => 
      `التطبيق تم تصميمه وبرمجته بالكامل بواسطة المهندس المبدع **يوسف أسامة** 🧑‍💻، وهو مهندس ذكاء اصطناعي (AIE - Artificial Intelligence Engineer). تم بناء هذا النظام الفائق باستخدام تقنيات متطورة مثل Next.js 16 و React 19 وقواعد بيانات Firebase السحابية. يمكنك متابعة أعماله والتواصل معه مباشرة عبر حسابه على إنستقرام: [youssef_osama04](https://instagram.com/youssef_osama04) 🌟.`
  },
  {
    id: "site",
...
export function classifyQueryWithML(
  ...
  // منع الأسئلة خارج النطاق للمستخدمين العاديين
  if (!isOwner) {
    const outOfScopeRegex = /(?:كاس العالم|كأس العالم|كورة|كوره|كرة|كره|رياضة|رياضه|اهلي|اهلى|زمالك|مباراة|مباره|لاعب|رونالدو|ميسي|سياسة|سياسه|حزب|انتخاب|رئيس|حكومة|حكومه|طبخ|اكل|أكل|وصفة|وصفه|مطبخ|اغاني|أغاني|اغنيه|أغنية|موسيقى|مسلسل|فيلم|افلام|أفلام|العاب|لعبة|لعبه|ببجي|بلايستيشن|برمجة|برمجه|بايثون|جافا|كودينج|برمجة عامة)/i;
    if (outOfScopeRegex.test(textClean)) {
      return {
        reply: `أهلاً بك يا ${userName}. أنا مساعدك الذكي لتطبيق "يقين"، واختصاصي محصور فقط في القرآن الكريم، التفسير، الفقه، والعلوم الإسلامية، وميزات التطبيق. لا يمكنني التحدث في مواضيع أخرى كـ المواضيع الرياضية أو العامة. كيف يمكنني مساعدتك في أمور دينك أو استخدام التطبيق اليوم؟ 🌸`,
        score: 1.0,
...
اكتب لي خيارك الآن **(أ، ب، ج، د)** للإجابة وحصاد الحسنات والنقاط! 🏆`,
  ...
  if (smallTalkMatch) {
    return {
      reply: isOwner 
        ...
        : `أهلاً بك يا ${userName}! 🌟 أنا مساعدك الذكي لتطبيق "يقين"، واختصاصي محصور في الإجابة عن الأسئلة الدينية (القرآن، التفسير، الفقه، الحديث) وتوضيح أقسام وميزات التطبيق. كيف يمكنني مساعدتك اليوم؟ 🌸`,
      score: 1.0,
      category: "تحية"
    };
  }
  ...
  if (canTalkMatch) {
    return {
      reply: isOwner
        ? `أيوه يا ${userName}! 😄 أنا مساعدك الذكي داخل التطبيق، وأقدر أتكلم معاك عادي في أي حاجة تحبها.`
        : `أنا هنا للتحدث معك ومساعدتك في كل ما يخص العلوم الإسلامية والدينية (القرآن، التفسير، الفقه، الحديث) وميزات تطبيق "يقين" فقط. لا يمكنني التحدث في مواضيع خارج هذا النطاق. كيف يمكنني مساعدتك اليوم؟ 🌸`,
      score: 1.0,
      category: "دردشة"
    };
  }
  ...
  if (whatCanYouDoMatch) {
    return {
      reply: `أنا أقدر أساعدك في كل حاجة تخص التطبيق: قراءة القرآن مع التفسير، الاستماع للتلاوات، معرفة مواقيت الصلاة، إنشاء فيديو قرآني، الأذكار اليومية، تصفح الملف الشخصي، وجمع النقاط. اسألني سؤال محدد زي: "أعمل فيديو ازاي؟" أو "فين صفحة المصحف؟" أو "ايه مواعيد الصلاة؟"`,
      score: 1.0,
  ...
  if (websiteLinkMatch) {
    return {
      reply: `رابط الموقع هو: https://yaqeen-app.vercel.app
...
إذا كان لديك سؤال حول أي جزء من المشروع أو ميزة داخل التطبيق، اكتب لي السؤال بالعامية أو العربية الفصحى وسأفهمه مباشرةً.`,
  ...
  if (askOwnerQuery) {
    return {
      reply: `التطبيق تم تصميمه وبرمجته بالكامل بواسطة المهندس المبدع **يوسف أسامة** 🧑‍💻، وهو مهندس ذكاء اصطناعي (AIE - Artificial Intelligence Engineer). تم بناء هذا النظام الفائق باستخدام تقنيات متطورة مثل Next.js 16 و React 19 وقواعد بيانات Firebase السحابية. يمكنك متابعة أعماله والتواصل معه مباشرة عبر حسابه على إنستقرام: [youssef_osama04](https://instagram.com/youssef_osama04) 🌟.`,
      score: 1.0,
      category: "المطور"
    };
  }
...
• **اسم المستخدم (Username):** ${userUsername || "غير مسجل"}
...
إذا كنت ترغب في تعديل بلدك، اكتب لي ببساطة: **"غير بلدي إلى [الدولة الجديدة]"** وسأقوم بحفظها لك فوراً! ✏️✨`,
  ...
  if (questionHintMatch && topicHint.length > 0) {
    if (!isOwner) {
      const allowedKeywords = /(?:قران|قرآن|مصحف|تفسير|صلاة|صلاه|قبلة|قبله|اذان|أذان|ذكر|اذكار|أذكار|سبحة|سبحه|تسبيح|فيديو|فديو|نقاط|ترتيب|شرف|ملف|موقع|تطبيق|يوسف|مطور|سورة|سوره|آية|ايه|حديث|فقه|دين|اسلام|إسلام)/i;
      if (!allowedKeywords.test(topicHint)) {
        return {
          reply: `أهلاً بك يا ${userName}. أنا مساعدك الذكي لتطبيق "يقين"، واختصاصي محصور فقط في القرآن الكريم، التفسير، الفقه، والعلوم الإسلامية، وميزات التطبيق. لا يمكنني التحدث في مواضيع أخرى. كيف يمكنني مساعدتك في أمور دينك أو استخدام التطبيق اليوم؟ 🌸`,
          score: 1.0,
          category: "خارج النطاق"
        };
      }
