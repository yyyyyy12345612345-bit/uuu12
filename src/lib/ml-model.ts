...
اكتب لي خيارك الآن **(أ، ب، ج، د)** للإجابة وحصاد الحسنات والنقاط! 🏆`,
  ...
  const smallTalkMatch = /(?:تجربتك اليوم|تجربت?ك|مسا|صباح|اهلا|أهلاً|ازيك|كيف حالك|ايه اخبارك|كيفك|عامل ايه|طمني|قوللي|مساء النور|صباح الخير)/i.test(textClean);
  if (smallTalkMatch) {
    return {
  ...
  const aiCapabilityMatch = /(?:انت ai|انت ذكي|هل انت ai|هل انت ذكي|انت بوت|هل انت بوت|انت مساعد)/i.test(textClean);
  if (aiCapabilityMatch) {
    return {
      reply: `نعم يا ${userName}, أنا مساعد ذكي مبني على نظام محلي داخل التطبيق، مش مجرد ردود ثابتة. أقدر أفهم أسئلة عن التطبيق والقرآن وأرد عليك بسرعة وبدون انتظار لخادم خارجي في بعض الحالات.`,
      score: 1.0,
      category: "قدرات الذكاء الاصطناعي"
    };
  }
...
لو عايز تبقى المحادثة أقرب لشات ذكي، قول لي بس:
• "عايز أعرف إزاي أعمل فيديو"
• "دلني على قسم المصحف"
• "إزاي أستخدم المكتبة الصوتية؟"
• "عايز أعرف مواقيت الصلاة في مدينتي"

أنا هنا علشان أرد عليك بشكل مباشر وأقرب للمحادثة العادية، مش بمعلومات ثابتة فقط.`,
    score: highestSimilarity,
    category: "افتراضي"
  };
}
