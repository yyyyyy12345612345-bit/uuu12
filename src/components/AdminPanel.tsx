      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        ...
        {activeTab === 'settings' && (
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-[3rem] border border-white/10 bg-slate-900/80 p-8 shadow-[0_35px_120px_rgba(15,23,42,0.18)]">
              <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
                <h2 className="text-2xl font-black">وضع الصيانة</h2>
              ...
              <div className="mt-6 grid gap-5">
                ...
                <div className="space-y-2 text-right">
                  <label className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">سبب الصيانة</label>
                  <input value={maintenanceMode.reason} onChange={e => setMaintenanceMode({ ...maintenanceMode, reason: e.target.value })} className="w-full rounded-3xl border border-white/10 bg-slate-950/90 p-4 text-right text-sm outline-none" placeholder="مثال: تحديث قاعدة البيانات" />
                </div>
                <div className="space-y-2 text-right">
                  <label className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">رسالة للمستخدمين</label>
                  <textarea value={maintenanceMode.message} onChange={e => setMaintenanceMode({ ...maintenanceMode, message: e.target.value })} rows={3} className="w-full rounded-3xl border border-white/10 bg-slate-950/90 p-4 text-right text-sm outline-none" placeholder="نعمل على تحسين التطبيق..." />
                </div>
                <div className="space-y-2 text-right">
                  <label className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">المدة المتوقعة</label>
                  <input value={maintenanceMode.duration} onChange={e => setMaintenanceMode({ ...maintenanceMode, duration: e.target.value })} className="w-full rounded-3xl border border-white/10 bg-slate-950/90 p-4 text-right text-sm outline-none" placeholder="مثال: ساعتين" />
                </div>
                <button onClick={handleSaveMaintenance} className="mt-4 w-full rounded-3xl bg-gradient-to-r from-amber-400 to-orange-500 px-5 py-4 text-black font-black transition hover:shadow-xl hover:shadow-amber-500/20">
                  حفظ وضع الصيانة
                </button>
              </div>
            </div>
