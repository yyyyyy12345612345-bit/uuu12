...
export function VerseDetailsModal({ verseKey, onClose, surahName }: VerseDetailsModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[3000] flex items-center justify-center p-4 md:p-10 animate-in fade-in duration-300 font-['Tajawal']`}>
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-3xl bg-card rounded-[3.5rem] shadow-xl border border-border overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-500">
        ...
        {/* Header */}
        <div className="relative z-10 p-8 md:p-12 border-b border-border flex items-center justify-between bg-gradient-to-b from-foreground/[0.03] to-transparent">
          <div className="flex flex-col text-right">
            <div className="flex items-center gap-2 mb-2">
                <div className="h-px w-8 bg-primary/30" />
                <span className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">تفسير الآية</span>
            </div>
            <h2 className="font-['Amiri'] text-3xl md:text-5xl font-black text-foreground">سورة {surahName}</h2>
            <div className="flex items-center gap-3 mt-2">
                <Star className="w-3 h-3 text-primary fill-primary opacity-40" />
                <p className="text-sm font-bold text-foreground/45 tracking-widest uppercase">الآية {verseKey.split(':')[1]}</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="w-14 h-14 rounded-2xl bg-foreground/5 hover:bg-foreground/10 flex items-center justify-center transition-all border border-border group"
          >
            <X className="w-6 h-6 text-foreground/45 group-hover:text-foreground group-hover:rotate-90 transition-all duration-300" />
          </button>
        </div>

        {/* Navigation Tabs */}
        <div className="relative z-10 flex px-8 md:px-12 py-6 gap-4 bg-foreground/[0.03]">
          {[
        ...
        <div className="relative z-10 flex-1 overflow-y-auto p-8 md:p-14 pb-20 no-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-8">
              ...
              <span className="text-[10px] font-black text-foreground/20 uppercase tracking-[0.4em]">جاري جلب التفسير...</span>
            </div>
          ) : (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
              
              {/* Verse Text Display */}
              <div className="mb-16 text-center p-10 md:p-14 bg-foreground/5 rounded-[3rem] border border-border relative group">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-card px-6 py-1.5 rounded-full border border-primary/40 text-[10px] font-black text-primary uppercase tracking-widest z-10 whitespace-nowrap shadow-xl">نص الآية الكريمة</div>
                <div className="absolute inset-0 bg-gradient-to-br from-primary/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                <p className="font-['Amiri'] text-4xl md:text-6xl font-bold text-foreground leading-[1.8] drop-shadow-2xl relative z-10" dir="rtl">
                  ...
                  {activeTab === 'tafsir' && (
                    <div className="space-y-10">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-foreground">تفسير السعدي</h3>
                            <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest">تيسير الكريم الرحمن</p>
                        </div>
                      </div>
                      <div 
                        className="text-2xl md:text-3xl leading-[2.4] text-foreground/80 font-medium font-arabic text-justify md:text-right"
                  ...
                  {activeTab === 'translation' && (
                    <div className="space-y-12">
                      {verseData?.translations?.map((t: any) => (
                        <div key={t.id} className="space-y-8 p-10 rounded-[3rem] bg-foreground/[0.03] border border-border">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-foreground">التفسير الميسر</h3>
                                <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest">مجمع الملك فهد</p>
                            </div>
                          </div>
                          <p className="text-2xl md:text-3xl leading-relaxed text-foreground/90 font-arabic font-bold text-right">
                            {t.text.replace(/<[^>]*>?/gm, '')}
                          </p>
                        </div>
                      ))}
