...
// THE MOST RELIABLE QURAN IMAGE SERVER (KSU.EDU.SA)
const PAGE_IMAGE_API = "https://quran.ksu.edu.sa/png_big/";
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pt-28 pb-40 px-4 md:px-20 relative z-10 font-arabic">
        <div className="max-w-5xl mx-auto space-y-12">
            <div className="text-center space-y-3">
                <h2 className="text-5xl md:text-7xl font-bold text-white uppercase tracking-tighter">طبعة الشمرلي</h2>
                <div className="h-1.5 w-32 bg-primary mx-auto rounded-full" />
            </div>

          <div className="grid grid-cols-1 gap-5 mt-12 pb-20">
            {surahsData.map((surah) => (
              <button
                key={surah.id}
                onClick={() => openSurah(surah)}
                className="group relative flex items-center justify-between p-7 rounded-[2rem] bg-white/[0.04] border border-white/5 hover:border-primary/40 hover:bg-primary/5 transition-all duration-500"
              >
                <div className="flex items-center gap-8">
                    <div className="w-16 h-16 rounded-2xl bg-black/60 flex items-center justify-center border border-white/10 group-hover:bg-primary/20 transition-all">
                        <span className="text-xl font-bold text-primary">{surah.id}</span>
                    </div>
                    <div className="text-right">
                        <span className="text-3xl font-bold text-white block group-hover:text-primary transition-colors">{surah.name}</span>
                        <span className="text-[10px] text-white/30 uppercase tracking-[0.3em] font-bold">Surah {surah.id}</span>
                    </div>
                </div>
                <BookOpen className="w-8 h-8 text-white/10 group-hover:text-primary transition-colors" />
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="relative h-full flex flex-col">
            <header className="h-20 shrink-0 flex items-center justify-between px-6 z-[610] bg-black/40 border-b border-white/5">
              ...
              <div className="flex items-center gap-4">
                 <button onClick={prevPage} className="p-3 bg-white/5 rounded-2xl hover:bg-primary/20"><ChevronRight className="w-7 h-7 text-white" /></button>
                 ...
                 <button onClick={nextPage} className="p-3 bg-white/5 rounded-2xl hover:bg-primary/20"><ChevronLeft className="w-7 h-7 text-white" /></button>
              </div>
            ...
            <div className="flex-1 relative flex items-center justify-center overflow-hidden bg-[#080808]">
                ...
                {isLoading && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#080808]">
                        ...
                        <p className="text-white/30 text-xs font-bold uppercase tracking-[0.5em]">Fetching Holy Page...</p>
                    </div>
                )}
                ...
                <img 
                    ...
                    className="h-full object-contain shadow-2xl rounded-lg animate-in zoom-in-95 duration-700"
