...
import { X, ChevronLeft, ChevronRight, Download, ExternalLink, Loader2, AlertCircle } from "lucide-react";

// TRULY RELIABLE SHAMRELY SOURCES
const PDF_SOURCES = [
  "https://ia801804.us.archive.org/3/items/ShamrelyQuran/",
    ...
    <div className="h-full flex flex-col bg-[#050505] relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pt-28 pb-40 px-4 md:px-14 relative z-10">
        <div className="max-w-4xl mx-auto space-y-6 text-center">
            ...
            <p className="text-primary/60 text-sm uppercase tracking-widest font-bold">Shamrely Edition - Cloud Stream</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-10">
            {surahsData.map((surah) => (
              <button
                ...
                className="group relative flex items-center justify-between p-6 rounded-3xl bg-white/[0.03] border border-white/5 hover:border-primary/40 hover:bg-white/[0.06] transition-all duration-500 overflow-hidden shadow-lg"
              >
      ...
      {selectedSurah && (
        <div className="fixed inset-0 z-[500] flex flex-col animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="relative h-full flex flex-col">
            <header className="h-20 shrink-0 flex items-center justify-between px-6 border-b border-white/10 glass-effect z-[510]">
              <div className="flex items-center gap-4">
                <button onClick={closePdf} className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 transition-all"><X className="w-5 h-5" /></button>
                <div className="flex items-center gap-2">
                    <button onClick={() => navigateSurah('prev')} className="p-2.5 bg-white/5 rounded-xl hover:bg-primary/20"><ChevronRight className="w-5 h-5" /></button>
                    <div className="text-center min-w-[120px]">
                        <h3 className="text-xl font-bold text-white font-arabic">سورة {selectedSurah.name}</h3>
                    </div>
                    <button onClick={() => navigateSurah('next')} className="p-2.5 bg-white/5 rounded-xl hover:bg-primary/20"><ChevronLeft className="w-5 h-5" /></button>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <a href={pdfUrl} target="_blank" rel="noreferrer" className="p-3 bg-primary/10 text-primary rounded-2xl border border-primary/20 hover:bg-primary/20 font-bold flex items-center gap-2">
            ...
            <div className="flex-1 bg-[#111] relative overflow-hidden flex items-center justify-center">
                {isLoading && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#111]">
                        ...
                        <p className="text-white/40 animate-pulse">جاري تحميل المصحف من السيرفر...</p>
                    </div>
                )}
                ...
                {/* Fallback button if Google Viewer fails or after 10s */}
                {!isLoading && (
                    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20">
                        <button 
                            onClick={() => {
                                if (sourceIndex < PDF_SOURCES.length - 1) {
                            ...
                            className="px-6 py-3 bg-white/5 border border-white/10 rounded-full text-xs text-white/60 hover:bg-white/10 hover:text-white transition-all flex items-center gap-2 shadow-2xl backdrop-blur-md"
                        >
                            <AlertCircle className="w-4 h-4" />
                            السورة لا تظهر؟ اضغط للتغيير لمصدر آخر
                        </button>
                    </div>
                )}
            </div>
