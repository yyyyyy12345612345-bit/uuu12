      ...
      if (response.ok) {
        ...
        if (text) {
          ...
          if (parsed.question && Array.isArray(parsed.options) && parsed.options.length === 4) {
            ...
            console.log(`[QUIZ API] Successfully generated question via Val Town at difficulty ${difficulty}`);
            return NextResponse.json(parsed);
          }
        }
      } else {
        console.error("Val Town quiz API failed with status:", response.status);
      }
    } catch (valErr: any) {
      console.error("[QUIZ API] Val Town quiz error:", valErr.message || valErr);
    }
