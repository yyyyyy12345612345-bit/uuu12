...
const getKeys = () => ({
  gemini: process.env.GEMINI_API_KEY || process.env.Value || process.env.VALUE || "",
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (keys.gemini) {
      const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:streamGenerateContent?key=${keys.gemini}`;
      ...
      if (res.ok && res.body) {
        // Simple proxy for streaming (SSE like) or just return the body
        // For simplicity and avoiding complex parser, we return the stream with appropriate headers
        // Since Gemini uses its own JSON streaming format, let's just do a normal fetch if we want pure text, 
        // or we handle the stream on the client. We'll handle it on the client.
