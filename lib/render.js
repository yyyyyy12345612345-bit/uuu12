...
export async function startRender(jobId, data) {
  ...
  try {
    ...
    const hasNetworkBg = backgroundUrl && 
      !backgroundUrl.startsWith("color:") && 
      !backgroundUrl.startsWith("gradient:") &&
    ...
    const wideFilter = isWide ? ",scale=-2:1080,pad=1920:1080:(1920-iw)/2:(1080-ih)/2:black" : "";
    ...
    if (isVideoBg) {
      ...
      ffmpegCmd = [
        ...
        `-filter_complex`, `"[0:v][1:v]overlay=0:0:shortest=1${wideFilter},format=yuv420p[vout]"`,
        `-map "[vout]" -map 2:a`,
