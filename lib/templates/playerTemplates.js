...
export function renderBrainrotDetox({ opacity = 1, elapsed = 0, total = 300, showTitle = true, titleText = "علاج التعفن الدماغي", showTimer = true, showProgressBar = true }) {
  ...
  return `
    <g opacity="${opacity}">
      ...
      <text x="360" y="530" font-family="'Cairo', 'Tajawal', sans-serif" font-size="52" font-weight="900" fill="#ffffff" text-anchor="middle" filter="url(#textGlow)">
        ${escapeXml(titleText || "علاج التعفن الدماغي")}
      </text>
      ` : ""}

      ${showTimer ? `
      <!-- Wide Bold Numbers with scale(1.18, 1) -->
      <g transform="translate(360, 695) scale(1.18, 1)">
        <text x="0" y="0" font-family="'Montserrat', 'DejaVu Sans', 'Arial Black', sans-serif" font-size="125" font-weight="900" fill="#ffffff" text-anchor="middle" letter-spacing="3" filter="url(#textGlow)">
          ${timeStr}
        </text>
      </g>
      ` : ""}
      ...
      <rect x="180" y="740" width="360" height="6" rx="3" fill="rgba(255, 255, 255, 0.2)" />
      <rect x="180" y="740" width="${360 * progressRatio}" height="6" rx="3" fill="#ffffff" filter="url(#whiteGlow)" />
      ` : ""}
    </g>
  `;
}
