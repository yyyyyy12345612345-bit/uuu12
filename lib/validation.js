...
export function validateRenderRequest(body) {
  ...
  // backgroundUrl اختياري في قوالب المشغلات والألوان والتدرجات
  if (backgroundUrl && typeof backgroundUrl === "string" && backgroundUrl.trim().length > 0) {
    const isColorOrGrad = backgroundUrl.startsWith("color:") || backgroundUrl.startsWith("gradient:");
    if (!isColorOrGrad && !validateUrl(backgroundUrl)) {
      errors.push("backgroundUrl رابط غير مسموح به");
    }
  }
